...
int main(int argc, char * argv[]) {
  ...
  hello();
   9:	e8 d4 02 00 00       	call   2e2 <hello>
  exit(-1);
...
putc(int fd, char c)
{
 ...
 36a:	e8 f3 fe ff ff       	call   262 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
{
  write(fd, &c, 1);
 3d6:	e8 87 fe ff ff       	call   262 <write>
...
putc(int fd, char c)
{
 ...
 446:	e8 17 fe ff ff       	call   262 <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
...
{
  write(fd, &c, 1);
 463:	e8 fa fd ff ff       	call   262 <write>
  int c, i, state;
  uint *ap;
...
putc(int fd, char c)
{
 ...
 520:	e8 3d fd ff ff       	call   262 <write>
...
putc(int fd, char c)
{
 ...
 570:	e8 ed fc ff ff       	call   262 <write>
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
...
morecore(uint nu)
{
 ...
 67a:	e8 4b fc ff ff       	call   2ca <sbrk>
  if(p == (char*)-1)
